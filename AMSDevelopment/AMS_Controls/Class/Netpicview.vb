...
Public Class PictureViewer
    ...
    Private Sub Host_MouseMove(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs)
        ...
        If DownPress = True Then
            Host.Cursor = New Cursor(My.Resources.MoveImage.Handle)
    ...
    Private Function DrawPic(ByVal ZoomX As Single, ByVal ZoomY As Single) As Boolean
        If IsNothing(srcBitmap) Then Return False
        ...
        If Host.Width > srcBitmap.Width * Zfactor Then
            ...
            BitBlt(desHDC, 0, 0, Brec.X, Host.Height, srcHDC, 0, 0, TernaryRasterOperations.WHITENESS)
            BitBlt(desHDC, Brec.Right, 0, Brec.X, Host.Height, srcHDC, 0, 0, TernaryRasterOperations.WHITENESS)
        Else
        ...
        If Host.Height > srcBitmap.Height * Zfactor Then
            ...
            BitBlt(desHDC, 0, 0, Host.Width, Brec.Y, srcHDC, 0, 0, TernaryRasterOperations.WHITENESS)
            BitBlt(desHDC, 0, Brec.Bottom, Host.Width, Brec.Y, srcHDC, 0, 0, TernaryRasterOperations.WHITENESS)
        Else
